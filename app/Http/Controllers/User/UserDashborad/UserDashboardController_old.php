...
//        exec('cd C:\laragon\www\softcodeapp\softcodeapp\api && php artisan queue:work', $output, $retval);
    ...
    public function catwiseShop(Request $request)
    {
        ...
        $shops = ServiceProfile::select('id', 'lat', 'lng', 'workstation_id', 'ws_cat_id', 'subscriber_id', 'user_id', 'name', 'email', 'mobile', 'img_name', 'cover_image', 'home_delivery', 'offline_sale', 'online_sale', 'address', 'short_bio', 'zip_code', 'city', 'country', 'status', 'profile_type', 'addedby_id', 'open', 'package_status')
            ->where('profile_type', 'business', 'expired_at')
            ->where('status', 1)
            ->where('paystatus', 1)
