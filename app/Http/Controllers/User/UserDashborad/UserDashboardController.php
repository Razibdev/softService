...
class UserDashboardController extends Controller
{
    ...
    //Razib start
    public function usershopDataItem($id){
        $itemcount = Serviceitem::where('service_profile_id',$id)->count();
        return response()->json(['item' => $itemcount]);
    }


    public function usershopDataProduct($id){
        $itemcount = ServiceProfileProduct::where('service_profile_id',$id)->where('status','approved')->where('active',true)->count();
        return response()->json(['item' => $itemcount]);
    }

    public function usershopFavorite($id){
       $data =  ServiceProfile::join('favourites', 'service_profiles.id', 'favourites.favourable_id')->where('service_profiles.id', $id)->where('favourites.user_id', Auth::id())->count();
       return response()->json(['item' => $data]);

    }


    public function usershopDataFetch(Request $request){


        $data = ServiceProfile::select('id', 'lat', 'lng', 'workstation_id', 'ws_cat_id', 'subscriber_id', 'user_id', 'name', 'email', 'mobile', 'img_name', 'cover_image', 'address', 'short_bio', 'zip_code', 'city', 'country', 'status','paystatus', 'profile_type', 'addedby_id', 'open', 'fixed_location', 'package_status','website_link')
        ->when($request->long and $request->lat, function($query) use($request){
            $query->addSelect(DB::raw("ST_Distance_Sphere(

                POINT('$request->long', '$request->lat'), POINT(lng,lat) 
                ) as distance"))
            ->orderBy('distance');
        })->with('category')
      ...
      return response()->json(['data' =>$data, 'subscriber' => $subscriber]);
    }




    public function getAllProductVueCart(){
        $data = ServiceProfileProduct::where('status','approved')->where('active',true)->get();
   ...
   //end job and vacancy announce
    ...
    public function SoftcomJobApplyForm()
    {
        ...
        return view('user.softcomjobapply.softcomjobapplyform',compact('user'));
    }
    ...
    public function SoftcomJobApplyList()
    {
        ...
        $count=$datas->count();
        if($count>0){
            return view('user.softcomjobapply.softcomapplylist',compact('user','datas'));
        }else{
            return view('user.softcomjobapply.softcomjobapplyform',compact('user'));
        }
    ...
    public function SoftcomJobCandidateApprovedList()
    {
        menuSubmenu('dashboard', 'softcomjobcandidate');
        $user = Auth::user();
        $datas=SoftcomJobCandidate::where('status',1)->paginate(24);
    ...
    public function AssignApplicantforServiceprofileStore(Request $request)
    {
       ...
       // dd($request->all());
        ...
        $check = ServiceProfileWorker::where('worker_id',$request->worker_id)->count();
        if($check>10){
            return redirect()->back()->with('warning', 'Your Already Apply');
